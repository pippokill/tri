 ...
 * @author pierpaolo
 */
public class RankDiv {

    private static final Logger LOG = Logger.getLogger(RankDiv.class.getName());
        ...
        try {
            ...
            if (cmd.hasOption("i") && cmd.hasOption("o")) {
                ...
                while (y < years.size()) {
                    ...
                    double mean = list.stream().mapToDouble(v -> v.getScore()).average().getAsDouble();
                    //double dev = list.stream().mapToDouble(v -> v.getScore()).reduce(0, (a, b) -> a + Math.pow(b - mean, 2));
                    //dev = Math.sqrt(dev / (double) list.size());
                    for (RankWords.ScoredWord sw : list) {
                        sw.setScore(Math.abs(sw.getScore() - mean));
                    }
                    ...
                    BufferedWriter writer = new BufferedWriter(new FileWriter(cmd.getOptionValue("o") + "/DIV_rank_" + years.get(y)));
                    for (RankWords.ScoredWord sw : list) {
